...
SELECT
    ...
    CASE WHEN i.type IN ('out_refund', 'in_refund') THEN -COALESCE(f.total_potencia, 0.0) ELSE COALESCE(f.total_potencia, 0.0) END AS importe_potencia_sin_descuento,
    CASE WHEN i.type IN ('out_refund', 'in_refund') THEN -COALESCE(energia_custom.total_energia, 0.0) ELSE COALESCE(energia_custom.total_energia, 0.0) END AS importe_energia_sin_descuento,
    CASE WHEN i.type IN ('out_refund', 'in_refund') THEN -COALESCE(generacio_custom.total_energia, 0.0) ELSE COALESCE(generacio_custom.total_energia, 0.0) END AS importe_generacio,
    f.total_reactiva AS importe_reactiva,
    CASE WHEN i.type IN ('out_refund', 'in_refund') THEN -COALESCE(excesos.suma, 0.0) ELSE COALESCE(excesos.suma, 0.0) END AS importe_exceso_potencia,
    CASE WHEN i.type IN ('out_refund', 'in_refund') THEN -COALESCE(no_iva_concepts.base_costs, 0.0) ELSE COALESCE(no_iva_concepts.base_costs, 0.0) END AS otros_sin_iva,
    CASE WHEN i.type IN ('out_refund', 'in_refund') THEN -COALESCE(iva_concepts.base_costs, 0.0) ELSE COALESCE(iva_concepts.base_costs, 0.0) END AS otros_con_iva,
    f.total_lloguers AS importe_alquiler,
    CASE WHEN i.type IN ('out_refund', 'in_refund') THEN -COALESCE(no_energy_invoices_with_vat.other_lines, 0.0) ELSE COALESCE(no_energy_invoices_with_vat.other_lines, 0.0) END AS otros_con_iva,
    CASE WHEN i.type IN ('out_refund', 'in_refund') THEN -COALESCE(no_energy_invoices_no_vat.other_lines, 0.0) ELSE COALESCE(no_energy_invoices_no_vat.other_lines, 0.0) END AS otros_con_iva,
    CASE WHEN i.type IN ('out_refund', 'in_refund') THEN -COALESCE(otros_cobros.cobros, 0.0) ELSE COALESCE(otros_cobros.cobros,0.0) END AS otros_cobros,
    CASE WHEN i.type LIKE '%%refund' THEN -COALESCE(it_iese.base, 0.0) ELSE COALESCE(it_iese.base, 0.0) END AS base_iese,
    CASE WHEN i.type LIKE '%%refund' THEN -COALESCE(it_iese.amount, 0.0) ELSE COALESCE(it_iese.amount, 0.0) END AS base_iese,
    CASE WHEN i.type IN ('out_refund', 'in_refund') THEN COALESCE(-iv_21.base, NULL) ELSE COALESCE(iv_21.base, NULL) END AS base_iva,
    CASE WHEN i.type IN ('out_refund', 'in_refund') THEN COALESCE(-iv_21.amount, NULL) ELSE COALESCE(iv_21.amount, NULL) END AS cuota_iva,
    CASE WHEN ives.total_taxes > 1 THEN 'varios' ELSE ives.max_tax_name END AS tipo_iva,
    CASE WHEN i.type LIKE '%%refund' THEN -i.amount_total ELSE i.amount_total END AS total_Factura
...
LEFT JOIN iv_21 ON iv_21.id = i.id
